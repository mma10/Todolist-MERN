...
export const addTask = task => dispatch => {
    ...
    axios.post('http://localhost:5000/api/item/' + token,task).then(res => {
        dispatch({
            type: "ADD",
            payload: res.data
        })
...
export const updateTask = (oldTask, newTask, id, e) => dispatch => {
    ...
    const token = localStorage.getItem('token');
    axios.put('http://localhost:5000/api/item/' + token + '/' + id, newTask).then((res) => {                
        dispatch({
            type: "UPDATE",    
            id,        
            payload: newTask
...
export const deleteTask = id => dispatch => {
    ...
    axios.delete('http://localhost:5000/api/item/' + token + '/' + id ).then(res => {       
        dispatch({
            type: "DELETE",
            id           
        });
